...
import { auth, signInWithPopup, signInWithRedirect, provider } from "../../firebase";
...
const AccountForm = ({ isSignup, setIsSignup }) => {
  ...
  // ✅ FIXED: Added country code state for flexible phone number support
  ...
  const [isAIMedLabAuth, setIsAIMedLabAuth] = useState(true);
  const setCurrAuthApp = [setIsAIMedLabAuth, setIsGoogleAuth];
  ...
  // ✅ FIXED: Updated phone validation to support variable length based on country code
  const validatePhoneNumber = (phoneNumber) => {
    // Accept 7-15 digit phone numbers (international standard)
  ...
  const getFirebaseAuthErrorMessage = (error) => {
    ...
    if (code === "auth/configuration-not-found") {
      return "Google Sign-In is not configured in your Firebase project. Enable Google provider in Firebase Console > Authentication > Sign-in method.";
    }

    if (code === "auth/popup-closed-by-user") {
      return "Google sign-in popup was closed before completing the flow.";
    }
  ...
  const handleGoogleRegister = async () => {
    try {
      ...
      console.error(error);
      setIsAlert("error");
      setAlertCont(getFirebaseAuthErrorMessage(error));
      setTimeout(() => {
        setIsAlert("");
      }, 1500);
    }
    ...
    try {
      ...
      try {
        ...
        if (popupError.code === 'auth/popup-blocked' || popupError.code === 'auth/popup-closed-by-user') {
          console.warn("Popup blocked or closed, using redirect method...");
          // Note: Redirect will reload the page after sign-in
          // You may need to handle the redirect result differently
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ FIXED: Use country code state instead of hardcoded "+91"
                      ...
                      <>
                        ...
                        <div className="mb-4">
                          ...
                          >
                            ...
                            <option value="+1-1">+1 (Mexico)</option>
                          </select>
                        </div>
                ...
                <div className="mt-12 w-full">
                  <div className="mb-8">
                    ...
                    <div className="relative flex justify-center">
                      <span className="absolute -top-4 text-center text-sm text-blue-1 bg-blue-3 px-4 dark:bg-blue-25">
                        {isSignupVisible ? "Sign Up using" : "Sign In using"}
                      </span>
                    </div>
                  </div>
                  <div className="rounded-md px-2 relative">
                    <div className="flex items-center justify-center gap-x-6">
                      <div className="relative flex h-[40px] w-[40px] items-center justify-center">
                        <input
                          ...
                          name="authMethod"
                          value="AIMedLabAuth"
                          className="peer z-10 h-full w-full opacity-0"
                          onChange={() => handleAuthApp(0)}
                          checked={isAIMedLabAuth}
                        />
                        <label
                          htmlFor="AIMedLabAuth"
                      ...
                      <div className="relative flex h-[41px] w-[41px] items-center justify-center">
                        <input
                          type="radio"
                          id="GoogleAuth"
                          name="authMethod"
